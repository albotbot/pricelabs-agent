...
import type { PricesResponse, PriceEntry } from "../types.js";
...
export function registerPriceTools(
  ...
  server.tool(
    ...
    "Fetch daily pricing data for a listing including dynamic prices, demand signals (colors/descriptions), booking status, and ADR comparisons. Returns computed fields: demand_level (human-readable), adr_vs_stly_pct.",
    {
    ...
    async (args) => {
      ...
      const result = await fetchWithFallback<PricesResponse>(
        cacheKey,
        () =>
          apiClient
        ...
        (data) => {
          ...
          const enrichedEntries = data.data.map((entry: PriceEntry) =>
            computePriceFields(entry),
          );
