...
import {
  ...
  GetChangeImpactInputSchema,
  GetUserConfigInputSchema,
  SetUserConfigInputSchema,
} from "../schemas/scale.js";
 ...
 */
export function registerScaleTools(
  server: McpServer,
  ...
  server.registerTool(
    ...
    {
      description:
        "Record an executed pricing change for revenue impact tracking. Creates a change_tracking entry with 7-day, 14-day, and 30-day follow-up check dates. Call this after every successful pricelabs_set_overrides, pricelabs_update_listings, or pricelabs_delete_overrides execution.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Query pending and completed revenue impact assessments from change tracking. Supports filtering by listing and/or pending-only (overdue checks). Use for daily impact review (SCALE-02) and 7/14/30 day follow-up workflows.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Read alert threshold configuration. Returns per-listing values merged with global defaults, or global-only if no listing specified. Always includes system defaults so the agent knows effective values. Use to check current thresholds before detect_underperformers (SCALE-04).",
  ...
  server.registerTool(
    ...
    {
      description:
        "Set an alert threshold configuration value. Validates bounds per config key. Set for a specific listing (with pms) or globally (omit both listing_id and pms). Returns the saved config entry. Use to customize alert sensitivity (SCALE-04).",
    ...
    async (params) => {
      try {
            ...
            content: [
              {
                ...
                text: JSON.stringify({
                  error:
                    "pms is required when listing_id is provided. Both must be set for per-listing config, or both omitted for global.",
                }),
              },
            ],
            isError: true,
            ...
            content: [
              {
                ...
                text: JSON.stringify({
                  error:
                    "listing_id is required when pms is provided. Both must be set for per-listing config, or both omitted for global.",
                }),
              },
            ],
            isError: true,
        ...
        if (bounds) {
          if (
            params.config_value < bounds.min ||
