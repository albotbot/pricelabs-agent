...
import type { NeighborhoodData, Listing } from "../types.js";
 ...
 */
export function registerNeighborhoodTools(
  server: McpServer,
  ...
  server.registerTool(
    ...
    {
      description:
        "Fetch neighborhood market data showing ADR, occupancy, and revenue at 25th/50th/75th/90th percentiles. Use for competitive positioning and base price calibration.",
    ...
    async (params) => {
      try {
        ...
        const cachedListing = cache.get<Listing>(`listing:${params.listing_id}:${params.pms}`);

        const result = await fetchWithFallback<NeighborhoodData>(
          cacheKey,
          async () => {
            const response = await apiClient.get<NeighborhoodData>(
              `/v1/neighborhood_data${queryString}`,
            );
