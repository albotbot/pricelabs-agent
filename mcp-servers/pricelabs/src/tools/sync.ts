...
export function registerSyncTools(
  ...
  server.registerTool(
    ...
    {
      description:
        "Trigger a price sync/push for a listing. Forces PriceLabs to recalculate and push updated prices to connected OTAs. DESTRUCTIVE -- only use when prices need immediate refresh. REQUIRES reason.",
    ...
    async (params) => {
      try {
        ...
        const response = await apiClient.post<unknown>("/v1/push_prices", {
          listing: listing_id,
          pms_name,
        });
  ...
  const AddListingToolInputSchema = z.object({
    ...
    pms_name: z
      .literal("bookingsync")
      .describe(
        "Only BookingSync PMS is supported for adding listings via the API",
      ),
    reason: z
      .string()
      .min(10)
      .describe(
        "Rationale for adding this listing (min 10 chars, required for audit trail)",
      ),
  });
  ...
  server.registerTool(
    ...
    {
      description:
        "Add a new listing to PriceLabs for price management. ONLY works for BookingSync PMS. DESTRUCTIVE -- creates a new managed listing. REQUIRES reason.",
 ...
 */
function formatWriteErrorResponse(
  error: unknown,
  operation: string,
): {
