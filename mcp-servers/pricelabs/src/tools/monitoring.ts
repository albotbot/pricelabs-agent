...
export function registerMonitoringTools(
  ...
  server.registerTool(
    ...
    {
      description:
        "Calculate booking pace at multiple cutoff windows (7/30/60/90 days) compared to same-time-last-year (STLY). Use for MON-03 (pace tracking) and MON-04 (pace alert detection).",
    ...
    async (params) => {
      try {
        ...
        const paceResults = cutoffDays.map((cutoff) => {
          ...
          let paceVsStlyPct: number | null = null;
