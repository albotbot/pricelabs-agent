...
export async function fetchWithFallback<T>(
  ...
  try {
    ...
    if (error instanceof RateLimitError) {
      return handleRateLimitFallback<T>(
        error,
        cacheKey,
        cache,
        rateLimiter,
 ...
 */
function handleRateLimitFallback<T>(
  ...
  if (cached) {
    ...
    return {
      ...
      } as ToolResponse<T>["meta"] & { note: string },
    };
  }
  ...
  throw new RateLimitError(
    error.retryAfterMs,
    `Rate limited with no cached data available. Retry in ~${retryMinutes} minutes.`,
  );
}
 ...
 */
function handleErrorFallback<T>(
  error: unknown,
  cacheKey: string,
  cache: TtlCache,
  rateLimiter: TokenBucketRateLimiter,
  ...
  if (cached) {
    ...
    return {
      ...
      meta: {
        ...
        outage_alert: true,
      } as ToolResponse<T>["meta"] & { note: string; outage_alert: boolean },
    };
  }
  ...
  throw new Error(
    `API error with no cached data (outage: ${outageMinutes} min): ${originalMessage}`,
  );
}
