...
export class PriceLabsApiClient {
    ...
    timeoutMs;
    headers;
    constructor(apiKey, rateLimiter, baseUrl = "https://api.pricelabs.co", timeoutMs = 300_000) {
        this.apiKey = apiKey;
    ...
    async request(method, path, body, retryCount = 0) {
        ...
        if (!rateCheck.allowed) {
            throw new RateLimitError(rateCheck.retryAfterMs ?? RETRY_CONFIG.defaultRetryAfterMs, "Local rate limit exceeded. Too many API calls.");
        }
        ...
        try {
            ...
            return await this.handleResponse(response, method, path, body, retryCount);
        }
        catch (error) {
            ...
            throw new ApiError(0, "", `Network error after ${RETRY_CONFIG.maxRetries} retries: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
        finally {
     ...
     */
    async handleResponse(response, method, path, body, retryCount) {
        ...
        if (status === 429) {
            ...
            throw new RateLimitError(RETRY_CONFIG.defaultRetryAfterMs, `Rate limited by API after ${RETRY_CONFIG.maxRetries} retries`);
        }
        // 401 Unauthorized -- NEVER retry, immediate fatal error
        if (status === 401) {
            throw new AuthError("Authentication failed. Verify that PRICELABS_API_KEY is valid.");
        }
