 ...
 */
export function registerRatePlanTools(server, apiClient, cache, rateLimiter) {
    server.registerTool("pricelabs_get_rate_plans", {
        ...
        try {
            ...
            const result = await fetchWithFallback(cacheKey, async () => {
                const response = await apiClient.get(`/v1/fetch_rate_plans${queryString}`);
                return response.data;
            }, cache, rateLimiter, RATE_PLANS_CACHE_TTL_MS);
            return {
