...
import { GetOverridesInputSchema, SetOverridesInputSchema, DeleteOverridesInputSchema, } from "../schemas/overrides.js";
 ...
 */
export function registerOverrideTools(server, apiClient, cache, rateLimiter) {
    // ---------------------------------------------------------------
    // Tool 1: pricelabs_get_overrides (read)
    // ---------------------------------------------------------------
    const getShape = GetOverridesInputSchema.shape;
    server.tool("pricelabs_get_overrides", "Fetch active date-specific overrides (DSOs) for a listing. Shows price adjustments, min-stay rules, and check-in/check-out restrictions per date.", {
    ...
    server.tool("pricelabs_set_overrides", "Create or update date-specific overrides (DSOs) for a listing. DSOs have the HIGHEST priority in PriceLabs and override ALL other settings including min price. REQUIRES reason for audit trail.", {
        ...
        for (const override of overrides) {
            if (override.price_type === "percentage" &&
        ...
        const fixedOverrides = overrides.filter((o) => o.price_type === "fixed");
        ...
        let writeResponse;
        try {
            writeResponse = await apiClient.post(`/v1/listings/${listing_id}/overrides`, { pms, overrides });
        }
        catch (error) {
    ...
    server.tool("pricelabs_delete_overrides", "Delete date-specific overrides for specified dates. Removes DSO pricing and restores dynamic pricing for those dates. REQUIRES reason for audit trail.", {
        ...
        try {
            await apiClient.request("DELETE", `/v1/listings/${listing_id}/overrides`, { pms, dates });
        }
        catch (error) {
            const message = error instanceof Error ? error.message : "Unknown error";
            return {
