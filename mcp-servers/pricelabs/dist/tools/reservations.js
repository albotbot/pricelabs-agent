 ...
 */
export function registerReservationTools(server, apiClient, cache, rateLimiter) {
    server.registerTool("pricelabs_get_reservations", {
        ...
        try {
            ...
            const result = await fetchWithFallback(cacheKey, async () => {
                const response = await apiClient.get(`/v1/reservation_data${queryString}`);
                return response.data;
            }, cache, rateLimiter, RESERVATIONS_CACHE_TTL_MS);
            return {
