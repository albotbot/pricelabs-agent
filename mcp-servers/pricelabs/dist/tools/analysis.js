 ...
 */
import { GetPortfolioKpisInputSchema, DetectUnderperformersInputSchema, } from "../schemas/analysis.js";
 ...
 */
export function registerAnalysisTools(server, db) {
    ...
    server.registerTool("pricelabs_get_portfolio_kpis", {
    ...
    server.registerTool("pricelabs_detect_underperformers", {
        ...
        try {
            ...
            const snapshotDate = params.snapshot_date ??
                db
                    .prepare("SELECT MAX(snapshot_date) FROM listing_snapshots")
                    .pluck()
                    .get() ??
            ...
            const marketMap = new Map(marketPositions.map((m) => [`${m.listing_id}:${m.pms}`, m]));
