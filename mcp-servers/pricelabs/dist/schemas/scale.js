...
export const GetChangeImpactInputSchema = z.object({
    ...
    pms: z
        .string()
        .optional()
        .describe("PMS name (e.g., 'airbnb', 'bookingsync'). Required if listing_id is provided."),
    pending_only: z
        .boolean()
        .optional()
        .describe("If true, only return changes with pending (overdue) check intervals. Default false."),
    limit: z
        .number()
        .optional()
...
export const RecordChangeInputSchema = z.object({
    ...
    affected_dates_start: z
        .string()
        .optional()
        .describe("Start of affected date range for DSO changes (YYYY-MM-DD). Optional for non-DSO changes."),
    affected_dates_end: z
        .string()
        .optional()
        .describe("End of affected date range for DSO changes (YYYY-MM-DD). Optional for non-DSO changes."),
    before_json: z
        .string()
...
export const GetUserConfigInputSchema = z.object({
    listing_id: z
        .string()
        .optional()
        .describe("Get config for specific listing (merged with global defaults). Omit for global-only."),
    pms: z
        .string()
        .optional()
        .describe("PMS name (e.g., 'airbnb', 'bookingsync'). Required if listing_id is provided."),
...
export const SetUserConfigInputSchema = z.object({
    ...
    listing_id: z
        .string()
        .optional()
        .describe("Set for specific listing. Omit for global default. Both listing_id and pms must be provided together or both omitted."),
    pms: z
        .string()
        .optional()
        .describe("PMS name (e.g., 'airbnb', 'bookingsync'). Required if listing_id is provided."),
