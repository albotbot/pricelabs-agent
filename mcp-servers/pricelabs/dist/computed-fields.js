...
function parseHealthScore(value) {
    if (value === null || value === undefined)
        return null;
 ...
 */
export function computeListingFields(listing) {
    const computed = {};
    // --- occupancy_gap_pct ---
    ...
    const daysSinceSync = computed.days_since_sync;
 ...
 * Never throws on bad input data.
 */
export function computePriceFields(priceEntry) {
    const computed = {};
    // --- demand_level ---
    ...
    if (bookingStatus !== null && bookingStatus !== undefined) {
        const statusLower = typeof bookingStatus === "string" ? bookingStatus.toLowerCase() : "";
        computed.is_booked =
            statusLower === "booked" ||
 ...
 * Never throws on bad input data.
 */
export function computeNeighborhoodFields(neighborhood, listing) {
    const computed = {};
    ...
    const futurePercentilePrices = neighborhood.data?.["Future Percentile Prices"];
    if (basePrice !== null &&
